<?php
    ...
    class typeq {
        ...
        public function listar(array $data){
        	...
        	$sql = "SELECT t.id ID, t.name FLOTA, t.description DESCRIPCIÓN,
                        CASE t.edo_reg 
                            WHEN '0' THEN 'INACTIVO'
                            WHEN '1' THEN 'ACTIVO'
                        ELSE 'no es un tipo' END 'ESTADO',                      
                        '<a href=\"editar\" rel=\"typeq\" action=\"upd\" title=\"Editar flota de equipos\" class=\"btn btn-sm btn-success\"><i class=\"fa fa-pencil\"></i></a>' MODIFICAR
					FROM ".BD_PREFI."typeEquipments t
					WHERE t.id > 0 ";
        ...
        public function editar(int $data){
            ...
            $sql = "SELECT t.id, t.name, t.description, t.edo_reg 
                    FROM ".BD_PREFI."typeEquipments t
                    WHERE t.id = ?
                    LIMIT 1;";
        ...
        public function guardar(){
            ...
            if( !empty($data->hidId) ){
                ...
                $resp = $this->crud->update($info,BD_PREFI.'typeEquipments',$where);
                ...
                $resp = $this->crud->insert($info,BD_PREFI.'typeEquipments');
